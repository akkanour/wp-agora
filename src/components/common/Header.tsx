...
export default function Header() {
  ...
  return (
    <>
      <header className="relative w-full z-50 bg-white font-body">
  <nav aria-label="Global" className="mx-auto flex max-w-7xl items-center justify-between p-4">
    <div className="flex lg:flex-1">
      <Link to="/" className="" onClick={() => handleLinkClick(null)}>
        ...
        <img alt="Logo" src="/logos-AT/AT-1.png" className="w-[200px]" />
      </Link>
    </div>
          ...
          {item.subMenu ? (
            <>
              <button
                ...
                className={`flex items-center gap-x-1 text-sm font-medium leading-6 ${selectedMenu === item.name ? 'text-[#024CAA]' : 'text-[#23374D]'}`}
              >
                {item.name}
            ...
            <Link
              to={item.href}
              className="text-sm font-medium leading-6 text-[#23374D] relative"
              onClick={() => handleLinkClick(item.name)}
            >
              {item.name}
              <span
  ...
  <Dialog open={mobileMenuOpen} onClose={() => setMobileMenuOpen(false)} className="lg:hidden font-body">
          ...
          <div className="space-y-2 pt-14">
            {menuItems.map((item) => (
              <Disclosure as="div" key={item.name} className="-mx-3">
                {item.subMenu ? (
                  <>
                    <DisclosureButton className="group flex w-full items-center justify-between rounded-lg py-2 pr-3.5 text-base font-medium leading-7 text-[#23374D] hover:bg-[#E5E5E5]">
                      {item.name}
                    ...
                    <DisclosurePanel className="mt-2 space-y-2 pl-6">
                      {item.subMenu.map((subItem) => (
                        <Link
                          key={subItem.name}
                          to={subItem.href}
                          className="block rounded-lg py-2 text-sm font-medium leading-7 text-[#23374D] hover:bg-[#024CAA] hover:text-white"
                          onClick={() => handleLinkClick(item.name)}
                        >
                          {subItem.name}
                        </Link>
                  ...
                  <Link
                    to={item.href}
                    className="-mx-3 block rounded-lg px-3 py-2 text-base font-medium leading-7 text-[#23374D] hover:bg-[#E5E5E5]"
                    onClick={() => handleLinkClick(item.name)}
                  >
                    {item.name}
                  </Link>
